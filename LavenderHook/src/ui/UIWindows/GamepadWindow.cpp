                  ...
                  bool& latched, bool shiftHeld, int idx)
{
    ...
    const ImVec4 baseCol = LavenderHook::Globals::use_polished_overlay
        ? ImVec4(0.086f, 0.078f, 0.110f, 0.95f)   // frosted panel tone
        : ImVec4(0.05f, 0.05f, 0.05f, 0.92f);
                     ...
                     bool& latched)
{
    ...
    if (polished) {
        // frosted panel + soft top sheen + accent border to match the theme
        dl->AddRectFilled(sp, br, C(22, 20, 28, 0.96f), 6.f);
        dl->AddRectFilled(sp, ImVec2(br.x, sp.y + size * 0.5f),
                          C(255, 255, 255, 0.04f), 6.f, ImDrawFlags_RoundCornersTop);
        ImU32 accent = ImGui::GetColorU32(ImVec4(MAIN_RED.x, MAIN_RED.y, MAIN_RED.z, 0.35f * drawAlpha));
        dl->AddRect(sp, br, accent, 6.f, 0, 1.5f);
        ImU32 ring = ImGui::GetColorU32(ImVec4(MAIN_RED.x, MAIN_RED.y, MAIN_RED.z, 0.30f * drawAlpha));
        dl->AddCircle(ctr, rad, ring, 48, 1.2f);
        dl->AddLine({ctr.x - rad, ctr.y}, {ctr.x + rad, ctr.y}, C(120, 120, 130, 0.30f));
        dl->AddLine({ctr.x, ctr.y - rad}, {ctr.x, ctr.y + rad}, C(120, 120, 130, 0.30f));
    }
    else {
...
void GamepadWindow::Render(bool wantVisible)
{
    ...
    ImGui::SetNextWindowSize(ImVec2(w, 36.0f * s + 100.0f), ImGuiCond_FirstUseEver);
...

    {
        ...
        ImGui::SetWindowSize(ImVec2(w, 36.0f * s + s_contentAnim + bottomPad));
    }

    ImVec2 gpPos = ImGui::GetWindowPos();
    ImVec2 gpSize = ImGui::GetWindowSize();
    ImGui::End();
    if (LavenderHook::Globals::use_polished_overlay)
        DrawWindowShadow(gpPos, gpSize, alpha);
    ImGui::PopStyleVar();
}
