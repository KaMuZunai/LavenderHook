...
namespace LavenderHook::UI::Windows {
    ...
    void MacroEditorWindow::Render(bool wantVisible)
    {
        ...
        // Polished title bar (sheen + accent divider when polished)
        {
            ImDrawList* dl = ImGui::GetWindowDrawList();
            ImVec2 wPos = ImGui::GetWindowPos();
            float  wW = ImGui::GetWindowWidth();
            float  hH = ImGui::GetFrameHeight() + 8.0f;
            float  fa = g_fade.Alpha();
            float  r = ImGui::GetStyle().WindowRounding;

            if (LavenderHook::Globals::use_polished_overlay) {
                dl->AddRectFilled(wPos, ImVec2(wPos.x + wW, wPos.y + hH * 0.55f),
                    IM_COL32(255, 255, 255, (int)(9.f * fa)), r, ImDrawFlags_RoundCornersTop);
                dl->AddLine(ImVec2(wPos.x + 1.0f, wPos.y + hH),
                            ImVec2(wPos.x + wW - 1.0f, wPos.y + hH),
                            LavenderHook::UI::Lavender::PolishedAccent(0.55f * fa), 1.0f);
            }
        }
