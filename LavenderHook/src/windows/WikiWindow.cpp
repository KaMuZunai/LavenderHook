...
namespace LavenderHook::UI::Windows {
    ...
    static ImVec4 kPillColor = ImVec4(0.4f, 0.3f, 0.6f, 0.8f);
    ...
    void WikiWindow::DrawCategoryTabs() {
        ...
        float tabH = ImGui::GetFrameHeight() + 4;
        ...
        for (const auto& cat : kCategories) {
            ...
            dl->AddRectFilled(pos, ImVec2(pos.x + tabW, pos.y + tabH), bg, 4);
        ...
        ImGui::SetCursorScreenPos(ImVec2(startX, curY + tabH + 4));
        ImGui::PopStyleVar();
        ...
        for (const auto& sub : subcats) {
            ...
            ImDrawList* dl = ImGui::GetWindowDrawList();
            ImVec4 bgCol = active ? ImVec4(0.5f, 0.4f, 0.7f, 0.8f) : ImVec4(0.2f, 0.2f, 0.3f, 0.5f);

            if (!active && ImGui::IsMouseHoveringRect(pos, ImVec2(pos.x + subW, pos.y + subH)))
                bgCol = ImVec4(0.35f, 0.3f, 0.5f, 0.7f);

            dl->AddRectFilled(pos, ImVec2(pos.x + subW, pos.y + subH), ImGui::GetColorU32(bgCol), 3);
        ...
        ImGui::SetCursorScreenPos(ImVec2(subStartX, subCurY + lastSubH + 4));
    }
    ...
    void WikiWindow::DrawItemCard(const ManifestEntry& entry, float cardWidth) {
        ...
        float textX = iconEnd.x + padding + favBtnSize + 8;
        float nameWidth = cardEnd.x - textX - 8;
        ...
        // Subtype pills
        float pillY = pos.y + padding + ImGui::GetFontSize() + 4;
        float pillX = textX;
        for (const auto& st : entry.subtype) {
            ImVec2 sts = ImGui::CalcTextSize(st.c_str());
            float pw = sts.x + 12, ph = sts.y + 2;
            dl->AddRectFilled(ImVec2(pillX, pillY), ImVec2(pillX + pw, pillY + ph),
                ImGui::GetColorU32(kPillColor), ph * 0.5f);
            dl->AddText(ImVec2(pillX + 6, pillY + 1), ImGui::GetColorU32(kPillTextColor), st.c_str());
            pillX += pw + 4;
        }

        // Favorite button - bigger, on the left side, centered vertically
        float favX = pos.x + iconSize + padding + 4;
        ...
        // Draw a filled circle background for the button
        ImVec2 fc = ImVec2(favX + favBtnSize * 0.5f, favY + favBtnSize * 0.5f);
        float fr = favBtnSize * 0.45f;
        ImU32 fbg = ImGui::GetColorU32(isFav ? ImVec4(0.3f, 0.2f, 0.0f, 0.8f) : ImVec4(0.15f, 0.15f, 0.2f, 0.6f));
        dl->AddCircleFilled(fc, fr, fbg);
        if (favHov) dl->AddCircle(fc, fr, ImGui::GetColorU32(ImVec4(0.6f, 0.6f, 0.6f, 0.4f)));

        ImVec4 sc = isFav ? kFavoriteColor : (favHov ? ImVec4(0.8f, 0.7f, 0.3f, 0.8f) : ImVec4(0.5f, 0.5f, 0.5f, 0.5f));
        const char* star = isFav ? "*" : "+";
        ImVec2 sts = ImGui::CalcTextSize(star);
        dl->AddText(ImVec2(favX + (favBtnSize - sts.x) * 0.5f, favY + (favBtnSize - sts.y) * 0.5f),
            ImGui::GetColorU32(sc), star);
        ...
        ImGui::SetCursorScreenPos(ImVec2(pos.x, cardEnd.y + 4));
        ImGui::PopID();
    }
    ...
    void WikiWindow::DrawItemDetail() {
        ...
        if (!loaded) {
            ...
            ImGui::SetCursorPos(ImVec2(ImGui::GetCursorPosX() + iconSize + 12, ImGui::GetCursorPosY()));
            ImGui::Text("%s", detailDisplayName.c_str());

            // Big star button next to name
            bool isFav = m_favorites.find(item.path) != m_favorites.end();
            ImGui::SameLine();
            float detFavSize = 28.0f;
            ImGui::SetCursorPosX(ImGui::GetCursorPosX() + 8);
            ImGui::InvisibleButton("##det_fav", ImVec2(detFavSize, detFavSize));
            ImVec2 detFavPos = ImGui::GetItemRectMin();
            ImVec2 detFavCenter(detFavPos.x + detFavSize * 0.5f, detFavPos.y + detFavSize * 0.5f);
            bool detFavClicked = ImGui::IsItemClicked();
            bool detFavHov = ImGui::IsItemHovered();
            ImDrawList* detDl = ImGui::GetWindowDrawList();
            detDl->AddCircleFilled(detFavCenter, detFavSize * 0.42f,
                ImGui::GetColorU32(isFav ? ImVec4(0.4f, 0.3f, 0.0f, 0.9f) : ImVec4(0.15f, 0.15f, 0.2f, 0.6f)));
            if (detFavHov)
                detDl->AddCircle(detFavCenter, detFavSize * 0.42f, ImGui::GetColorU32(ImVec4(0.6f, 0.6f, 0.6f, 0.4f)));
            const char* detStar = isFav ? "*" : "+";
            ImVec2 detSts = ImGui::CalcTextSize(detStar);
            detDl->AddText(ImVec2(detFavCenter.x - detSts.x * 0.5f, detFavCenter.y - detSts.y * 0.5f),
                ImGui::GetColorU32(isFav ? kFavoriteColor : (detFavHov ? ImVec4(0.8f, 0.7f, 0.3f, 0.8f) : ImVec4(0.5f, 0.5f, 0.5f, 0.5f))), detStar);
            if (detFavClicked) {
            ...
            if (!item.subtype.empty()) {
                ...
                for (const auto& st : item.subtype) {
                    ...
                    dl->AddRectFilled(ppos, ImVec2(ppos.x + pw, ppos.y + ph),
                        ImGui::GetColorU32(kPillColor), ph * 0.5f);
                    dl->AddText(ImVec2(ppos.x + 6, ppos.y + 1), ImGui::GetColorU32(kPillTextColor), st.c_str());
    ...
    void WikiWindow::RenderFavoriteOverlay() {
        ...
        ImGui::SetNextWindowSizeConstraints(ImVec2(160, 0), ImVec2(FLT_MAX, FLT_MAX));
        ...
        if (ImGui::Begin("##fav_overlay", nullptr, flags)) {
            ...
            dl->AddRectFilled(winPos, ImVec2(winPos.x + winSize.x, winPos.y + winSize.y),
                ImGui::GetColorU32(ImVec4(0, 0, 0, 0.3f * alpha)), 6);
            ...
            for (const auto& favPath : m_favorites) {
                ...
                if (hovered) {
                    dl->AddRectFilled(pos, ImVec2(pos.x + size.x, pos.y + size.y),
                        ImGui::GetColorU32(ImVec4(0.2f, 0.2f, 0.3f, 0.5f)), 3);
                }

                float iconS = 24.0f;
                auto favIconIt = m_iconCache.find(favPath);
                if (favIconIt != m_iconCache.end() && favIconIt->second.IsValid()) {
                    dl->AddImage(favIconIt->second.id, ImVec2(pos.x + 2, pos.y + 2),
                        ImVec2(pos.x + 2 + iconS, pos.y + 2 + iconS));
                }
                ...
                float textX = pos.x + iconS + 6;
                float textW = size.x - iconS - 6;
